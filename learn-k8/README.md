...
3. Builds Docker image from `learn-k8/Dockerfile` and pushes to `gcr.io/$GCP_PROJECT/my-app:$GITHUB_SHA`.
4. Retrieves GKE credentials and runs `helm upgrade --install` using the chart in `learn-k8/helm-chart`.
...
helm upgrade --install my-app learn-k8/helm-chart --set image.repository=gcr.io/your-project/my-app --set image.tag=local
```
