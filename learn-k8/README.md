...
- `GCP_SERVICE_ACCOUNT` — JSON service account key with permissions: Container Engine Admin, Storage Admin (for pushing images) and Service Account User. Store the JSON contents.
- `GKE_CLUSTER` — cluster name (e.g. `my-gke-cluster`).
- `GKE_LOCATION` — cluster zone or region (e.g. `us-central1-a` or `us-central1`).

How it works
1. Push to `main` triggers workflow.
2. Workflow authenticates to GCP using `GCP_SERVICE_ACCOUNT` and `GCP_PROJECT`.
